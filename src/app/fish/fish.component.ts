...
export class FishComponent implements OnInit, OnChanges {
  ...
  ngOnInit(): void {
    this.force = forceSimulation(this.nodes)
      ...
      .force('collision', forceCollide(14.5))
  ...
  setupNodes() {
    ...
    this.svg
      ...
      .attr('text-anchor', (d: any) => {
        return !d.depth ? 'end' : d.horizontal ? 'middle' : 'middle';
      })
      .attr('dy', (d: any) => {
        return d.horizontal ? '.25em' : d.region === 1 ? '1em' : '-0.2em';
      })
      .text((d: any) => {
        return d.name;
      });

    this.svg.selectAll('text').call(this.wrap, 10);
  ...
  linkDistance(l: any) {
    const linkScale = scaleLog().domain([1, 5]).range([55, 10]);
